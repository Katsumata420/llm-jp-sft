...
class ModelArguments:
    ...
    lora_target_model: str = field(
        default="llama",
        metadata={"help": "The target model name for LoRA training."},
    )
    lora_target_modules: Optional[list[str]] = field(
        default=None,
        metadata={"help": "The target modules for LoRA training."},
    )
    lora_r: int = field(
        default=8,
        metadata={"help": "The rank parameter for LoRA training."},
    )
    lora_alpha: int = field(
        default=16,
        metadata={"help": "The alpha parameter for LoRA training."},
    )
    ...
    def __post_init__(self):
        if self.lora_target_model and self.lora_target_modules is None:
            if self.lora_target_model == "llama":
            ...
            elif self.lora_target_model == "llm-jp":
                self.lora_target_modules = [
                    "c_attn",
                    "c_proj",
                    "c_fc",
                ...
                raise ValueError(f"Unknown target model {self.lora_target_model}.")
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        elif data_args.train_file is not None and data_args.validation_file is None:
